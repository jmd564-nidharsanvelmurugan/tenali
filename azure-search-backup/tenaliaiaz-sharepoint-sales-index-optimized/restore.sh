...
curl -X PUT "$BASE_URL/datasources/azureblob-sharepoint-sales-datasource?api-version=$API_VERSION" \
  -H "api-key: $ADMIN_KEY" \
...
curl -X PUT "$BASE_URL/skillsets/azureblob-sharepoint-sales-skillset?api-version=$API_VERSION" \
  -H "api-key: $ADMIN_KEY" \
  -H "Content-Type: application/json" \
  -d @skillset.json
...
echo "Creating index..."
curl -X PUT "$BASE_URL/indexes/tenaliaiaz-sharepoint-sales-index-optimized?api-version=$API_VERSION" \
  -H "api-key: $ADMIN_KEY" \
  -H "Content-Type: application/json" \
  -d @index.json
...
curl -X PUT "$BASE_URL/indexers/azureblob-sharepoint-sales-indexer?api-version=$API_VERSION" \
  -H "api-key: $ADMIN_KEY" \
  -H "Content-Type: application/json" \
  -d @indexer.json
