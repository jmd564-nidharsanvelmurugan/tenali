...
echo "Restoring ai-proposal-workspace-index..."

curl -X PUT "$BASE_URL/datasources/ai-proposal-workspace-datasource?api-version=$API_VERSION" \
  -H "api-key: $ADMIN_KEY" -H "Content-Type: application/json" -d @datasource.json

curl -X PUT "$BASE_URL/skillsets/ai-proposal-workspace-skillset?api-version=$API_VERSION" \
  -H "api-key: $ADMIN_KEY" -H "Content-Type: application/json" -d @skillset.json

curl -X PUT "$BASE_URL/indexes/ai-proposal-workspace-index?api-version=$API_VERSION" \
  -H "api-key: $ADMIN_KEY" -H "Content-Type: application/json" -d @index.json

curl -X PUT "$BASE_URL/indexers/ai-proposal-workspace-indexer?api-version=$API_VERSION" \
